...
const MEDIA_CANDIDATES: Record<
  Exclude<MediaMappableField, "__ignore__">,
  string[]
> = {
  title: [
    "title",
  ...
  tags: ["tags", "tag", "etiket", "etiketler", "labels"],
  posterUrl: ["poster", "poster url", "posterurl", "afiş", "afis"],
};
...
export function guessMediaField(header: string): MediaMappableField {
  ...
  for (const [field, list] of Object.entries(MEDIA_CANDIDATES)) {
    if (list.some((c) => n.includes(norm(c)) || norm(c).includes(n)))
      return field as MediaMappableField;
  }
  return "__ignore__";
}

export const MAPPABLE_MEDIA_FIELDS = Object.keys(
  MEDIA_FIELD_LABELS,
) as MediaMappableField[];
...
const CANDIDATES: Record<Exclude<MappableField, "__ignore__">, string[]> = {
  ...
  rating: ["rating", "puan", "yıldız", "yildiz", "stars"],
  tags: ["tags", "tag", "etiket", "etiketler", "labels"],
};
...
export function isBookField(
  f: MappableField,
): f is Exclude<MappableField, "__ignore__"> {
  return f !== "__ignore__";
}
