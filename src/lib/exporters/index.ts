...
import { sanitizeSpreadsheetValue } from "../validation";
...
function toRow(
  ...
  fields.forEach((f) => {
    ...
    if (f === "tagIds")
      v = (b.tagIds ?? []).map((id) => tagNames[id] ?? id).join("; ");
    r[label] = sanitizeSpreadsheetValue(v ?? "");
  });
  return r;
}
...
export async function exportXlsx(
  ...
  const rows = books.map((b) => toRow(b, fields, tagNames));
...
export function exportJson(
  ...
  downloadBlob(
    new Blob([JSON.stringify({ books: data }, null, 2)], {
...
function toMediaRow(
  ...
  fields.forEach((f) => {
    ...
    if (f === "tagIds")
      v = (m.tagIds ?? []).map((id) => tagNames[id] ?? id).join("; ");
    r[label] = sanitizeSpreadsheetValue(v ?? "");
  });
  return r;
}

export async function exportMediaXlsx(
  media: Media[],
  fields: (keyof Media)[],
  filename: string,
  tagNames: Record<string, string> = {},
  ...
  const XLSX = await import("xlsx");
  const rows = media.map((m) => toMediaRow(m, fields, tagNames));
  const ws = XLSX.utils.json_to_sheet(rows);
  const wb = XLSX.utils.book_new();
...
export function exportMediaJson(
  ...
  downloadBlob(
    new Blob([JSON.stringify({ media: data }, null, 2)], {
      type: "application/json",
    }),
    filename,
  );
