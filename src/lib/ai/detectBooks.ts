...
export function isAiBridgeAvailable(): boolean {
  return typeof window !== 'undefined' && !!window.kutuphanem?.ai;
}
...
export async function downscaleImage(file: File): Promise<{ base64: string; mimeType: string }> {
  const dataUrl = await new Promise<string>((resolve, reject) => {
    ...
    img.onerror = () => {
      URL.revokeObjectURL(url);
      reject(new Error('Bu görsel açılamadı. iPhone HEIC fotoğraflarını JPG/PNG olarak kaydedip tekrar dene.'));
    };
    img.src = url;
  });

  const comma = dataUrl.indexOf(',');
  return { base64: comma >= 0 ? dataUrl.slice(comma + 1) : dataUrl, mimeType: 'image/jpeg' };
}

/** Bir fotoğraftan kitap listesi çıkarır: anahtarı doğrular, görseli küçültür, köprüyü çağırır. */
export async function detectBooksFromImage(file: File): Promise<DetectBooksResult> {
  const apiKey = useSettings.getState().geminiApiKey?.trim();
  if (!apiKey) {
    return { ok: false, error: 'Önce Ayarlar → Yapay Zekâ bölümünden ücretsiz Gemini anahtarını ekle.' };
  }
  if (!isAiBridgeAvailable()) {
    return { ok: false, error: 'Bu özellik masaüstü uygulamasında çalışır (tarayıcı önizlemesinde değil).' };
  }
  ...
  try {
    return await window.kutuphanem!.ai.detectBooks({ apiKey, imageBase64: img.base64, mimeType: img.mimeType });
  } catch (e) {
