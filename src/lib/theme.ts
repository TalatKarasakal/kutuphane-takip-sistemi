...
export function applyTheme(settings: AppSettings) {
  void window.kutuphanem?.theme.set(settings.theme);
...
export function subscribeNativeTheme(settings: AppSettings): () => void {
  if (settings.theme !== "system" || !window.kutuphanem) return () => undefined;
  return window.kutuphanem.theme.onChange((mode) => {
