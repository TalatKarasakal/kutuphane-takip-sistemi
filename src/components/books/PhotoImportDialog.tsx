...
import {
  Upload, Loader2, AlertTriangle, Sparkles, Settings, ImagePlus,
  Trash2, CheckCircle2,
} from 'lucide-react';
...
export function PhotoImportDialog({ open, onClose, onOpenSettings }: Props) {
  ...
  const reset = () => {
    setStep('pick'); setFileName(''); setError(''); setNotice(''); setRows([]);
  };
  ...
  const onFile = async (file: File) => {
    setError(''); setNotice(''); setFileName(file.name); setStep('detecting');
    ...
    if (res.books.length === 0) {
      setNotice('Bu fotoğrafta kitap algılanamadı. Sırtların/kapakların daha net göründüğü bir fotoğrafla tekrar dene.');
      setStep('pick');
      return;
    }
    const seen = existing();
  ...
  return (
    <Modal
      ...
      onClose={close}
      size="xl"
      title={'Fotoğraftan Ekle' + (fileName && step !== 'pick' ? ` · ${fileName}` : '')}
      footer={
        step === 'review' ? (
          <>
            <button className="btn btn-ghost" onClick={reset}>Baştan</button>
            <button className="btn btn-ghost" onClick={close}>Kapat</button>
            <button className="btn btn-primary" onClick={doAdd} disabled={includedCount === 0}>
              {includedCount > 0 ? `${includedCount} Kitabı Ekle` : 'Ekle'}
            </button>
          </>
        ) : (
          <button className="btn btn-ghost" onClick={close}>Kapat</button>
...
function PickStep({
  hasKey, error, notice, onFile, onOpenSettings,
}: {
  hasKey: boolean;
  error: string;
  notice: string;
  ...
  if (!hasKey) {
    return (
      <div className="py-8 text-center">
        ...
        <div className="font-medium mb-1">Önce ücretsiz Gemini anahtarını ekle</div>
        <div className="text-sm text-muted max-w-md mx-auto mb-4">
          Bu özellik fotoğraftaki kitapları tanımak için Google'ın ücretsiz Gemini servisini kullanır.
          Google AI Studio'dan kredi kartı gerekmeden ücretsiz bir anahtar alıp Ayarlar'a yapıştırman yeterli.
        </div>
  ...
  return (
    <div className="py-4">
      ...
      >
        ...
        <div className="font-medium mb-1">Kitap fotoğrafı seç ya da buraya sürükle-bırak</div>
        <div className="text-sm text-muted mb-4">.jpg · .png · .webp — raftaki sırtların/kapakların net göründüğü bir fotoğraf en iyi sonucu verir</div>
        <input
          type="file"
        ...
        />
        <span className="btn btn-primary"><Upload size={15} /> Fotoğraf Seç</span>
      </label>
      <div className="text-xs text-muted mt-3 flex items-start gap-2">
        <Sparkles size={14} className="mt-0.5 shrink-0 text-primary" />
        <span>Yapay zekâ kitapları tanır, künyeyi (yayınevi, sayfa, kapak) Google Books'tan tamamlar; eklemeden önce listeyi gözden geçirip onaylarsın.</span>
      </div>
    </div>
  );
}
...
function DetectingStep() {
  return (
    <div className="py-16 text-center">
      ...
      <div className="text-sm text-muted">Kitaplar tanınıyor ve künyeleri tamamlanıyor. Bu birkaç saniye sürebilir.</div>
    </div>
  );
}

function ReviewStep({
  rows, onPatch, onApplyAll,
}: {
  rows: ReviewRow[];
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <div className="inline-flex border border-border rounded-lg bg-surface overflow-hidden">
            <button className="px-3 py-1.5 text-sm hover:bg-surface2" onClick={() => onApplyAll('okunacak')}>Okunacak</button>
            <button className="px-3 py-1.5 text-sm hover:bg-surface2 border-l border-border" onClick={() => onApplyAll('satin-alinacak')}>Satın Alınacak</button>
          </div>
        </div>
        <div className="chip bg-emerald-500/15 border border-emerald-500/20 text-emerald-700 dark:text-emerald-300">
          <CheckCircle2 size={12} /> {rows.filter((r) => r.include && r.title.trim()).length} seçili
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.rid} className={`border-t border-border ${r.include ? '' : 'opacity-50'}`}>
                <td className="px-2 py-2 text-center">
                  <input
                    type="checkbox"
                    className="accent-[rgb(var(--primary))] w-4 h-4"
                    checked={r.include}
                    onChange={(e) => onPatch(r.rid, { include: e.target.checked })}
                  />
                </td>
                <td className="px-2 py-2">
                  {r.coverUrl
                    ? <img src={r.coverUrl} alt="" className="w-8 h-11 object-cover rounded shadow-sm" />
                    : <div className="w-8 h-11 rounded bg-surface2 border border-border" />}
                </td>
                <td className="px-3 py-2">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    {r.duplicate && <span className="chip bg-amber-500/15 border border-amber-500/20 text-amber-700 dark:text-amber-300">listede var</span>}
                    {!r.matched && <span className="text-[11px] text-muted">künye bulunamadı</span>}
                    {r.publicationYear && <span className="text-[11px] text-muted">{r.publicationYear}</span>}
                  </div>
                </td>
                <td className="px-3 py-2">
                  <input
                  ...
                  />
                  {r.publisher && <div className="text-[11px] text-muted mt-1 truncate">{r.publisher}</div>}
                </td>
                <td className="px-3 py-2">
                  <select
                    className="input py-1"
                    value={r.status}
                    onChange={(e) => onPatch(r.rid, { status: e.target.value as BookStatus })}
                  >
      ...
      <div className="text-xs text-muted flex items-start gap-2">
        ...
        <span>Yapay zekâ yanılabilir — başlık/yazarı kontrol et, gerekirse düzelt. İşaretli olanlar eklenecek.</span>
      </div>
    </div>
  );
}
